...
class UndergraduateFullPage extends StatelessWidget {
  const UndergraduateFullPage({super.key});
          ...
          children: [
            ...
            _buildInfoNote(
              "The undergraduate admissions at Nanjing University are carried out by the Undergraduate Admissions Office under the Student Affairs Office. "
                  "The admissions office is also responsible to the Nanjing University undergraduate admissions committee and leading group.\n\n"
                  "Currently, the university admits about 3,300 undergraduate students each year from 31 provinces in China and from Hong Kong, Macao, and Taiwan.",
              isDark,
            ),

            // Types of Recommended Students
            ...
            _buildBulletList([
              ...
              "Special Cultivation Plan for Ethnic Minority Regions (e.g., Tibet and Xinjiang)",
            ...
            SingleChildScrollView(
              ...
              child: DataTable(
                headingRowColor: MaterialStateProperty.resolveWith<Color?>((states) {
                  // Dark/light mode header color
                  return isDark
                      ? AppColor.surfaceColor.withOpacity(0.8)
                      : BrandGradient.luxury.colors.first.withOpacity(0.2);
                }),
                dataRowColor: MaterialStateProperty.resolveWith<Color?>((states) {
                  // Optional: alternating row color
                  if (states.contains(MaterialState.selected)) {
                    return AppColor.accentGold.withOpacity(0.3);
                  }
                  return isDark
                ...
                columns: const [
                  DataColumn(label: Text("Major/Program")),
                  DataColumn(label: Text("Category")),
                  DataColumn(label: Text("Length (year)")),
                  DataColumn(label: Text("School")),
                ],
                rows: undergraduatePrograms.map((program) {
                  return DataRow(
                    cells: [
                      DataCell(Text(program['major']!)),
                      DataCell(Text(program['category']!)),
                      DataCell(Text(program['length']!)),
                      DataCell(Text(program['school']!)),
                    ],
                  );
                }).toList(),
              ),
            ...
            _buildContactCard(
              ...
              website: "Official WeChat: ndzsxlj\nVisit online website for undergraduate admissions",
              isDark: isDark,
            ),
